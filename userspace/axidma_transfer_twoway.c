...
int read_reg(unsigned long target) {
	int fd;
                         ...
                         char *output_path)
{
	...
	int count = 0;
	int send_size = 0;
	void *inptr = axidma_malloc(dev, 1024);
	void *outptr = axidma_malloc(dev , 1024);
	while(1) {
		if((reg_val = read_reg(0x43c00010)) > 0x3800){
			continue;
	    }
		count += 1024;
        send_size = count > trans->input_size ? trans->input_size%1024 : 1024;
		memset(inptr, 0x0, 1024);
		memcpy(inptr, trans->input_buf + count - 1024, send_size);
		memset(outptr, 0x0, 1024);

		rc = axidma_twoway_transfer(dev, trans->input_channel, inptr,
				send_size, trans->output_channel, outptr,
				send_size, true);
		if (rc < 0) {
			fprintf(stderr, "DMA read write transaction failed.\n");
			goto free_output_buf;
		...
		// Write the data to the output file
//		printf("Writing output data to `%s`.\n", output_path);
		rc = robust_write(trans->output_fd, outptr, send_size);
	...
	axidma_free(dev, inptr, 1024);
	axidma_free(dev, outptr, 1024);
free_output_buf:
